...
export const ManageDemoModal = ({ course, onClose, showToast, refresh }) => {
  const confirm = useConfirm();
   ...
   useEffect(() => {
      if (editingDemo) {
         setValue('title', editingDemo.title || '');
         setValue('video_url', editingDemo.video_url || '');
         setValue('duration', editingDemo.duration || '');
      } else {
         reset();
      }
   }, [editingDemo, setValue, reset]);
   ...
   const onSubmit = async (data) => {
      ...
      try {
         ...
         const body = isEdit
            ? { title: data.title, video_url: data.video_url, duration: data.duration }
            : { course_id: course.course_id, title: data.title, video_url: data.video_url, duration: data.duration };
         ...
         if (res.ok) {
            ...
            if (detailRes.ok) {
               ...
               setDemos(detailData.course.demo || []);
            }
            reset();
            setEditingDemo(null);
               ...
               <div style={{ borderRight: '1px solid var(--color-border)', paddingRight: '2.5rem' }}>
                  <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                     <Section title={editingDemo ? "Update Demo" : "Add Demo Video"}>
                        <FormInput label="Video Title" {...register('title')} error={errors.title} placeholder="e.g. Module 1 Preview" />
                        <FormInput label="Video URL" {...register('video_url')} error={errors.video_url} placeholder="https://vimeo.com/..." />
                        <FormInput label="Duration" {...register('duration')} error={errors.duration} placeholder="e.g. 5:30" />
                     </Section>
                     <div style={{ display: 'flex', gap: '1rem' }}>
                        {editingDemo && (
                           <button type="button" onClick={() => { setEditingDemo(null); reset(); }} style={{ flex: 1, padding: '1rem', borderRadius: '1rem', border: '1px solid var(--color-border)', fontWeight: '800', cursor: 'pointer', background: 'white' }}>Cancel</button>
                        )}
